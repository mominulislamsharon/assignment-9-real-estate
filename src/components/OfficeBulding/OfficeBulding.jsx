...
const OfficeBulding = ({ office }) => {
  ...
  return (
    <div className="card bg-[#fafafa]  shadow-xl">
      <figure>
        <img className="h-[350px] w-[480px] rounded-lg"
          src={image}
          alt="Shoes"
        />
      ...
      <div className="card-body">
        ...
        <p className="text-lg font-semibold pt-3">{segment_name}</p>
        <h2 className="text-2xl pt-3 font-bold">{estate_title}</h2>
        <p className="text-base font-medium text-slate-500 pt-3 border-b-2 pb-6">{description}</p>
        <h2 className="text-lg bg-[#dbe61d] w-48 h-10 mt-3 pl-3 pt-1.5 rounded-full  font-medium">{price}</h2>
        <div className="flex  mt-3 items-center gap-3 bg-[#ffcc41] bg-opacity-30 h-10 w-36 rounded-2xl pl-2">
            <FaShareSquare></FaShareSquare>
            <p className="text-lg font-semibold ">{area}</p>
        </div>
        <div className="card-actions border-t-2 pt-3  mt-3">
